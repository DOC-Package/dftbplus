...
contains
  ...
  #:endif
    ...
    if (allocated(this%elecConstraint)) then
      ...
      ! Write final constraint potential to file (Added by Hideaki Takahashi)
      call writeFinalVc(this%elecConstraint, constrConverged)
    end if
...
#:endif
  ...
  !> Write final constraint potential to file (Added by Hideaki Takahashi).
  subroutine writeFinalVc(elecConstraint, tConverged)

    use dftbp_dftb_elecconstraints, only : TElecConstraint

    !> Electronic constraint object
    type(TElecConstraint), intent(in) :: elecConstraint

    !> Whether constraints converged
    logical, intent(in) :: tConverged

    real(dp), allocatable :: Vc(:), deviation(:)
    integer :: fd, iConstr, nConstr
    character(len=*), parameter :: fileName = "final_Vc.dat"

    Vc = elecConstraint%getVc()
    deviation = elecConstraint%getDeviation()
    nConstr = size(Vc)

    if (nConstr == 0) return

    open(newunit=fd, file=fileName, status="replace", action="write")
    write(fd, "(A)") "# Final constraint potential Vc and deviation (N_calc - N_target)"
    write(fd, "(A)") "# Generated by DFTB+ with electronic constraints"
    if (tConverged) then
      write(fd, "(A)") "# Status: CONVERGED"
    else
      write(fd, "(A)") "# Status: NOT CONVERGED"
    end if
    write(fd, "(A)") "#"
    write(fd, "(A8, 2A24)") "# Index", "Vc [Ha]", "Deviation"
    do iConstr = 1, nConstr
      write(fd, "(I6, 2ES24.15)") iConstr, Vc(iConstr), deviation(iConstr)
    end do
    close(fd)

    write(stdOut, "(A,A)") ">> Final constraint potential saved in ", fileName

  end subroutine writeFinalVc
